...
async def execute_run(run_id: int, user_id: int, config: dict, db_url: str) -> None:
    ...
    4. Update run status
    """
    from shortlist.api.db import _clean_url

    engine = create_async_engine(_clean_url(db_url))
    ...
    try:
        ...
        # Set the API key in environment for the pipeline's LLM module
        ...
        await update_run(progress={"phase": "collecting"})

        # Run sync pipeline in a thread with temp SQLite
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)

            def _run_sync():
                from shortlist.pipeline import run_pipeline
                return run_pipeline(pipeline_config, project_root)

            brief_path = await asyncio.to_thread(_run_sync)

            await update_run(progress={"phase": "saving results"})
            ...
            async with async_session() as session:
                for row in jobs:
                    # Check if job already exists for this user (by description_hash)
