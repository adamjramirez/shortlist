...
- **Tri-state for external checks** — any periodic check against a third-party URL/API returns `True` (explicit live), `False` (explicit gone), `None` (unknown / transient). Caller acts only on explicit signals. Never collapse "unknown" into "gone." Applied in `expiry.check_job_url()`. See SL-019 in `CONSTRAINTS.md`.
- **Recency-skip before external calls** — before any periodic checker call, skip if the resource's `last_seen` is within the last 24h. Transient network errors vastly outweigh genuine removal on fresh data. Zero schema, one line, highest-leverage defense against false positives. Preferred over N-strike counting for the common case.
- **Expiry checker is a continuous proxy-traffic source** — `scheduler.run_expiry_checks` fires every `TICK_INTERVAL` seconds and calls `expiry.check_expiry_batch(limit=N)`. With `TICK_INTERVAL=300` and `limit=5`, that's ~60 HEAD/hr; LinkedIn URLs route through Decodo. Tunables: `expiry._run_batch(limit=)` and `scheduler.TICK_INTERVAL`. Before tuning either *up*, count `jobs WHERE is_closed=false AND last_seen < NOW() - INTERVAL '24 hours'` — that's the eligible population that drives sustained rate. Recency-skip protects fresh rows but not the long tail of LinkedIn jobs that fall off search result drift and never get their `last_seen` refreshed.
- **Kill-switch deploy pattern** — when recovering from a faulty automated process: (1) ship a tiny env-var gate at the entry point (`if os.environ.get("DISABLE_X") == "1": return`), (2) activate via `fly secrets set DISABLE_X=1` (VM restart), (3) run remediation (SQL reopen, data fix, etc.), (4) ship the real fix, (5) `fly secrets unset DISABLE_X`. Log the disabled state once at WARN so it's visible in `fly logs`. Used 2026-04-16 for url_check remediation; commits 88a4ca9 + 3acd4cf.
...
- **Scored field pattern** — adding a new field to an existing LLM scorer call: (1) add to `ScoreResult` dataclass, (2) add `{field}` placeholder to `SCORING_PROMPT_TEMPLATE`, (3) add to `SCORE_SCHEMA` required list, (4) extract and validate in `parse_score_response()`, (5) add `field=build_field(config)` to the `.format()` call in `build_scoring_prompt()`, (6) add `if score_result.field: updates["field"] = score_result.field` in pipeline scoring loop, (7) add the column to `shortlist/db.py` `SCHEMA` (`CREATE TABLE jobs`) **AND** to `_JOBS_LATE_COLUMNS` so existing local SQLite DBs get the column added on next `init_db()` — the Postgres/web path needs an Alembic migration in `alembic/versions/` instead. For fields with non-trivial criteria, extract a `build_field_criteria(config)` pure function so criteria stay in sync with the user's profile and can be reused for backfill.
...
- ❌ Treating non-200 HTTP status as "gone" in expiry/health checks — `return resp.status_code == 200` silently destroys data. 403 bot challenges, 429 rate limits, 5xx proxy flakes, and 301/302 login redirects are NOT closure signals. Only an explicit 404 (or source-specific body signal like Ashby's title="Jobs") means gone. Everything else = unknown, skip. See SL-019 in `CONSTRAINTS.md`. Incident: 73 live jobs closed 2026-04-16, `expiry.py:61` pre-fix.
- ❌ Assuming the 24h recency-skip caps expiry-check volume — LinkedIn jobs collected once but never re-collected (search-result drift) keep their `last_seen` stale forever, so they re-fire the HEAD check on every loop indefinitely. With ~3000 such rows and a 60s tick × 20 batch, that's 1200 HEAD/hr through Decodo 24/7. Real fixes: lower batch size + tick rate (shipped 2026-04-26), or track `last_check_at` separately and skip if checked recently regardless of `last_seen`. Discovery 2026-04-26.
- ❌ Conflating "skipped", "unknown", and "exception" into one `errors` counter — makes aggregate logs misleading. "16 errors" in a scheduler log actually meant 15 healthy recency-skips + 1 real error. Separate counters per outcome type.
- ❌ Assuming `fly secrets set/unset` is instant — every call triggers a VM restart (30-60s). Account for this when sequencing multi-step prod operations. Add a startup WARN log for any long-lived env-var gate so its state is visible in `fly logs`.
...
- ❌ Adding a column to `shortlist/db.py` `SCHEMA` without also adding it to `_JOBS_LATE_COLUMNS` — `CREATE TABLE IF NOT EXISTS` is a no-op when the table already exists, so existing SQLite DBs never get the new column. First `shortlist run` after the change crashes with `sqlite3.OperationalError: no such column: <name>`. Always pair schema additions with a `_JOBS_LATE_COLUMNS` entry so older DBs get an `ALTER TABLE ADD COLUMN` on next `init_db()`. Burned 2026-05-05 (PR #22 added scoring fields, first run after merge crashed on `salary_basis`).
...
- ❌ Returning 502 for LLM/provider errors when behind Cloudflare — Cloudflare intercepts 502/503/504 and replaces the response body with its own HTML error page. `resp.json()` fails in the frontend; user sees Cloudflare branding instead of our message. Use 422 for all app-level errors that need to reach the client as JSON. Reserve 502 only for when we are genuinely acting as a proxy and a true upstream is unreachable.
- ❌ Closing greenhouse/lever/ashby jobs based on `last_seen` alone — requires an active observer in `career_page_sources` (status='active', last_checked_at within 7 days, last_jobs_count > 0). Without an observer, "haven't seen it in 3 days" carries no signal — we wouldn't have seen it anyway. Burned 2026-05-17: 1,610+ false closes across 7 companies (Anthropic, Anduril, Figma, Workato, Airbnb, Five9, Samsara) that were batch-seeded once but had no recurring observer → `last_seen` froze at the seed date → 3-day sweep cutoff fired → every job from those companies closed silently over 6 weeks. Discovered via CSVFirst QA cross-check. Fix: `pgdb.mark_stale_jobs` Pass 1 now requires the observer predicate (tri-state, like SL-019 for expiry).
- ❌ Only calling `saveApiKeyOrThrow()` when `apiKey` is truthy in `handleAnalyze` — a model-only change (user switches model dropdown without entering a new key) never gets persisted before generate. Server reads stale model from DB. Always call `saveApiKeyOrThrow()` unconditionally before the LLM generate call — it saves `model: llmModel` alongside any key and is the sync point even with no new key.
- ❌ Reading only `encrypted_api_key` (legacy single-key) in the generate endpoint without checking `api_keys.{provider}` — users who saved keys via the per-provider flow get "no API key" errors. Pattern: `detect_provider(model)` → `api_keys.get(provider) or llm_config.get("encrypted_api_key")`. Same pattern as `tailor.py`'s `_configure_llm`. Keep them in sync.
- ❌ Using `gemini-2.0-flash` (non-versioned) as a default model — deprecated for new users with billing-enabled GCP projects; returns 404 "no longer available to new users." Use `gemini-2.5-flash` everywhere. Incident 2026-04-20: user 10 could not complete analyze flow because model default was stale across profile.py, tailor.py, worker.py, and profile/page.tsx. Also: a Gemini 429 with `limit: 0` on `generate_content_free_tier_input_token_count` = billing-enabled GCP project (free-tier quota removed, not exhausted). Fix is to get a key from aistudio.google.com, NOT enable billing.
